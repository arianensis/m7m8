...
public class Star implements Serializable {
    ...
    public static double[] sphericalToCubic (double[] spheriCoords) {
        ...
        // first we use declination to calculate the horizontal and vertical components
        double horDist = distance * Math.cos(dec);
        // the vertical component is already the Y coordinate
        double Y = distance * Math.sin(dec);
        // finally we use the horizontal component with the right ascension to calculate the projection on the plane
        double X = horDist * Math.cos(ra);
        double Z = horDist * Math.sin(ra);
    ...
    public static double[] cubicToSpherical (double[] cubeCoords) {
        ...
        double dist = Meridia.modulus(X,Y,Z);
        // the right ascension is the horizontal component of the angle, so it depends on X and Z and is not affected by Y
        // if the Z is the sinus and the X is the cosinus, Z/X is the tangent, so the angle will be arctan(Z/X)
        double ra = Math.toDegrees(Math.atan(Z/X));

        // the declination is the vertical angle, but we need the projection of the vector to calculate it
        // with Pythagoras theorem, we can get the horizontal distance
        double horDist = Meridia.modulus(X,Z);
        // and then use this, which is the contiguous cathetus, and the total distance, which is the hypotenuse
        double dec = Math.toDegrees(Math.acos(horDist/dist));
